...
class WPL_CustomUpdater {
	...
	public function check_for_plugin_update( $checked_data ) {
		...
		// $wpl_logger->info('check_for_plugin_update('.$this->plugin_slug.'): '.$this->api_url);
	...
	public function plugin_api_call( $def, $action, $args ) {
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( $res === false )
				$res = new WP_Error( 'plugins_api_failed', __( 'An unknown error occurred','wplister' ), $request['body'] );
		}
	...
	public function wpl_prepare_request( $action, $args, $new_license_key = false ) {
		...
		return array(
			'body' => array(
				...
				'domain'  => $this->domain,
				'channel' => get_option('wplister_update_channel'),
	...
	public function activate_license( $license_key, $email ) {
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( $res === false )
				$res = new WP_Error( 'activate_license_failed', __( 'An unknown error occurred','wplister' ), $request['body'] );
		}
	...
	public function deactivate_license( $license_key, $email ) {
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( $res === false ) {
				$res = new WP_Error( 'deactivate_license_failed', __( 'An unknown error occurred','wplister' ), $request['body'] );
			}
