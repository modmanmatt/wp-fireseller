...
class TransactionsModel extends WPL_Model {
	...
	function insertOrUpdate( $data, $hasVariations, $VariationSpecifics ) {
		...
		if ( $transaction ) {
			...
			if ( get_option( 'wplister_create_orders' ) == '1' ) {
				...
				$this->logger->info( 'updated order #'.$wp_order_id.' for transaction - transaction id #'.$data['transaction_id'] );
			}
			...
			if ( get_option( 'wplister_create_orders' ) == '1' ) {
				...
				$this->logger->info( 'created order #'.$wp_order_id.' for transaction - transaction id #'.$data['transaction_id'] );

				// add history record
				$history_message = "Order #$wp_order_id was created";
	...
	public function completeOrder( $session, $id, $data ) {
		...
		if ( trim( @$data['TrackingNumber'] ) != '' ) {
			...
			$ShippedTime = date( 'Y-m-d', $data['ShippedTime'] ) . 'T08:00:00Z';
		...
		$this->logger->info('completeOrder(): '.$id);
		$this->logger->info('ItemID: '.$req->ItemID);
		$this->logger->info('TransactionID: '.$req->TransactionID);
		...
		if ( $this->handleResponse($res) ) {
			$success = true;
			$this->logger->info( sprintf("Order %s updated successfully.", $id ) );
		} else {
			$this->logger->error( "Error on CompleteSale(): ".print_r( $res, 1 ) );			
