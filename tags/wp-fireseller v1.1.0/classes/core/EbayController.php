...
class EbayController {
    ...
    public function completeOrder( $order_id, $data ){        
        $sm = new TransactionsModel();
        $transaction = $sm->getTransactionByOrderID( $order_id );
        if ( $transaction ) {
            $response = $sm->completeOrder( $this->session, $transaction['id'], $data );         
        } else {
            // build response
            $response = new stdClass();
            $response->success      = false;
            $response->error        = 'no transaction found for order #' . $order_id ;
            $this->logger->error( 'no transaction found for order_id ' . $order_id );
        }
        return $response;
    }
